    ...
    async def recv(self) -> MediaStreamTrack:
        ...
        try:
            ...
            if results.pose_landmarks:
                ...
                # Example: Map pose landmarks to actuator movements
                ...
                # Robot control logic
                # if left_wrist_y < left_shoulder_y:  # Left hand raised
                #     print("Raising left shoulder!")
                #     actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                #     print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
                #     move_joint_a_little("left_shoulder_pitch", MOVE_DEGREES)
                if right_wrist_y < right_shoulder_y:  # Right hand raised
                    print("Raising right shoulder!")
                    actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                    print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
                    move_joint_a_little("right_shoulder_pitch", MOVE_DEGREES)
                if right_wrist_y > right_shoulder_y:  # Right hand lowered
                    print("Raising right shoulder!")
                    actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                    print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
                    move_joint_a_little("right_shoulder_pitch", -1 * MOVE_DEGREES)
                if left_wrist_x > left_shoulder_x:  # Left hand left
                    print("Raising left shoulder!")
                    actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                    print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
                    move_joint_a_little("right_elbow_yaw", MOVE_DEGREES)
                if right_wrist_x > right_shoulder_x:  # Move elbow to the right
                    print("Raising right shoulder!")
                    actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                    print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
                    move_joint_a_little("right_elbow_yaw", -1 * MOVE_DEGREES)
                # if right_wrist_x < right_shoulder_x:  # Move elbow to the right
                #     print("Raising right shoulder!")
                #     actuator_id = ACTUATOR_NAME_TO_ID["left_shoulder_pitch"]
                #     print(f"moving actuator {actuator_id} by {MOVE_DEGREES} degrees")
